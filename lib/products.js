...
var BASE_URL_product = "https://middle-layer.gomix.me/v1.2/21/products?limit="
var BASE_URL_category = "https://middle-layer.gomix.me/v1.2/21/products?"
var BASE_URL_CATEGORY = "http://d3u6s2iyc5cpge.cloudfront.net/xmls/dev/cat_"
...
exports.convert_category_data = function(category, callback){
  var URL = BASE_URL_CATEGORY + category + ".xml"
  request(URL, function(err, response, body){
    if (!err){
...
exports.limit_cat_data = function (data, limit, offset, category, callback) {
  ...
  for (var i = offset ; i< offset+limit; i++) {
    ...
    if (i === limit + offset) {
      ...
      data.metadata.links.next = BASE_URL_category + limit + "&offset=" + (offset+limit) + "&category=" + category
      console.log(data.metadata.links.next)
      records.push(data.records[i])
    }

  }
  data.metadata.links.self = BASE_URL_product + limit + "&offset=" + offset + "&category=" + category
  data.metadata.links.first = BASE_URL_product + limit + "&offset=0" + "&category=" + category
...
exports.limit_data = function (data, limit, offset, callback) {
  ...
  for (var i = offset ; i< offset+limit; i++) {
    if (i === limit + offset) {
      ...
      data.metadata.links.next = BASE_URL_product + limit + "&offset=" + (offset+limit)
      console.log(data.metadata.links.next)
      records.push(data.records[i])
    }
    data.metadata.links.self = BASE_URL_product + limit + "&offset=" + offset
    data.metadata.links.first = BASE_URL_product + limit + "&offset=0"
    data.records_count = data.records.length
    data.metadata.records_count = data.records.length
  }
  data.records = records
